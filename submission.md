...
Reward prediction error (RPE) theory holds that reward signals reflect the difference between received and predicted rewards, a computational account originally linked to dopaminergic activity in non-human primates (Schultz, Dayan & Montague, 1997). In humans, reward prediction errors have also been shown to predict momentary subjective well-being, such that outcomes that exceed expectations are associated with greater happiness than equivalent outcomes that were already expected (Rutledge et al., 2014). We apply RPE theory to Qwen3.6-27B, combining activation probing on affect-neutral gambles with independently constructed emotion-concept readouts and a sequential risk-choice task. We find that signed RPE is strongly separable in the model's residual stream (AUROC 0.985), and that the model's emotion-concept readout tracks the reward-versus-expectation comparison itself rather than either component independently (effect ratios 1.11 and 1.25; all permutation p < 10^-4). We further find that positive versus negative prior outcomes are associated with a +0.19-logit shift toward subsequent risk-taking (p ≈ 10^-4), although our intervention experiments do not yet establish that the identified RPE representation causally drives this behavioural effect. Recent outcome/expectation history may therefore be an internal state variable worth understanding and monitoring in agentic systems.

## 1. Introduction

As large language models (LLMs) increasingly take actions in the world through agentic systems, understanding the mechanisms behind those actions can reveal pre-existing biases and suggest ways of counteracting them. If a model's recent outcome history shifts its subsequent risk preferences — as prior outcomes shift human choices — then that history is an internal state variable worth monitoring in deployed agents. Considering an LLM as a digital mind also invites human- and animal-inspired questions about well-being: where such a signal would come from, and how it would influence subsequent action.
...
3. Sofroniew et al. (2026). "Emotion Concepts and their Function in a Large Language Model." *arXiv:2604.07729.* <!-- TODO (team): full author list for the reference entry -->
...
7. Berg (2026). "Why Learning Requires Feeling." *AAAI Spring Symposium Series, 8*(1), 227–233. https://doi.org/10.1609/aaaiss.v8i1.42547 <!-- TODO (team): author initials for the reference entry -->
...
For the widened geometry analysis, we constructed emotion-concept vectors for 111 emotion words plus one style-control pseudo-word (a control sharing the generated stories' style but carrying no target emotion concept), at 24 stories each; 2,684 of the 2,688 generated stories passed the target-naming filter. The word set was organized into appraisal-structured families, including outcome-linked positive and negative concepts and matched non-outcome controls.
